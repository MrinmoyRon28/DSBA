...
#--Q2.Write a query to display and Show inventory status of products
...
case
when p1.product_quantity_avail=0 then 'Out of stock'
when p1.product_quantity_avail<=10 then 'Low stock'
when p1.product_quantity_avail between 11 and 30 then 'In stock'
...
when p1.product_quantity_avail between 21 and 80 then 'In stock'
...
when p1.product_quantity_avail between 16 and 50 then 'In stock'
when p1.product_quantity_avail>=51 then 'Enough stock'
end
end as inventory_product
from product p1 inner join product_class p2 on p1.product_class_code=p2.product_class_code;

#--Q3.Write a query to show the number of cities in all countries other than USA & MALAYSIA, with more than 1 city, in the descending order of CITIES. 

select count(city) as city_except,country from address group by country having city_except>1 
and country not in ('USA','Malaysia') order by count(city) desc;

#--Q4.Write a query to display the customer_id,customer full name ,city,pincode,and order details

select c.customer_id,concat(c.customer_fname , " " , c.customer_lname) as customer_full_name,a.city,a.pincode,
o1.order_id,p2.product_class_desc,p1.product_desc,(o2.product_quantity*p1.product_price) as subtotal
from online_customer c inner join address a on c.address_id=a.address_id inner join order_header o1 on 
c.customer_id=o1.customer_id inner join order_items o2 on o1.order_id=o2.order_id inner join product p1
on o2.product_id=p1.product_id inner join product_class p2 on p1.product_class_code=p2.product_class_code
where a.pincode not like '%0%' and o1.order_status='Shipped' order by customer_full_name,subtotal;

#--Q5.Write a Query to display which item has been bought along with it maximum no. of times.

select o.product_id,p.product_desc,sum(o.product_quantity) as total_quantity from product p 
inner join order_items o on p.product_id=o.product_id
where o.order_id IN (select order_id from 
order_items where product_id=201) group by p.product_id,p.product_desc
order by total_quantity desc limit 1;

#--Q6.Write a query to display all customers even if they have not ordered any item.

select c.customer_id,concat(c.customer_fname ," ", c.customer_lname) as customer_name,c.customer_email,
o1.order_id,p.product_desc,o2.product_quantity,(o2.product_quantity*p.product_price) as subtotal
from online_customer c left join order_header o1 on c.customer_id=o1.customer_id left join order_items o2
on o1.order_id=o2.order_id left join product p on o2.product_id=p.product_id;

#--Q7.Write a query to display the optimum carton

SELECT carton_id, (len * width * height) AS carton_vol 
FROM carton
where (len * width * height) >(select sum(p.len * p.width * p.height * o.product_quantity)
from product p inner join order_items o on p.product_id=o.product_id
where o.order_id=10006) limit 1;

#--Q8. Write a query to display details of customers who bought more than ten products

select c.customer_id,concat(c.customer_fname , " " , c.customer_lname) as customer_fullname,
o.order_id,o1.product_quantity from online_customer c inner join order_header o on 
c.customer_id=o.customer_id and o.order_status='Shipped' and o.payment_mode in('Credit Card','Net Banking')
inner join order_items o1 on o.order_id=o1.order_id group by 
o1.order_id having sum(o1.product_quantity)>10;

#--Q9. Write a query to display the order_id, customer id and cutomer full name of customers 
#--starting with the alphabet "A" along with (product_quantity) as total quantity of products shipped for
#--order ids > 10030

select o.order_id,c.customer_id,concat(c.customer_fname , " " , c.customer_lname) as full_name,
sum(o1.product_quantity) as total_quantity
from online_customer c inner join order_header o on c.customer_id=o.customer_id 
and concat(c.customer_fname , " " , c.customer_lname) like 'A%' and o.order_status='Shipped'
inner join order_items o1 on o.order_id=o1.order_id group by o1.order_id having o1.order_id>10030;

#--Q10. Write a query to display product class description ,total quantity
#--and show which class of products have been shipped highest(Quantity) to countries outside India 
#--other than USA

select p1.product_class_desc,p1.product_class_code,sum(p2.product_quantity) as total_quantity,
sum(p2.product_quantity*p3.product_price) as total_value from product_class p1 inner join
product p3 on p1.product_class_code=p3.product_class_code inner join order_items p2 
on p3.product_id=p2.product_id inner join order_header o1 on p2.order_id=o1.order_id 
and o1.order_status='Shipped'
inner join online_customer c on o1.customer_id=c.customer_id
inner join address a on c.address_id=a.address_id where a.country not in('USA','India')
group by p1.product_class_desc,p1.product_class_code
order by total_quantity desc limit 1;
